...
$target_user_id = isset($input['target_user_id']) ? intval($input['target_user_id']) : null;

if (!$form_id || $form_id <= 0 || !$target_user_id || $target_user_id <= 0) {
...
try {
    ...
    if (intval($form['user_id']) !== $user_id) {
        ...
        echo json_encode(['error' => 'Seul le créateur peut retirer les accès']);
        exit;
    }

    $stmt = $pdo->prepare('DELETE FROM survey_access WHERE form_id = ? AND user_id = ?');
    $stmt->execute([$form_id, $target_user_id]);

    header('Content-Type: application/json');
    echo json_encode(['success' => true]);
} catch (PDOException $e) {
    http_response_code(500);
    echo json_encode(['error' => 'Erreur lors du retrait d\'accès']);
}
